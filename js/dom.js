...
// // - innerHTM은 보안상 취약하다고 알려져 있고, 따라서 insetAdjjacenHTML 사용을 권장한다. 은 자식요소의 파싱 과정이 생략 되기 때문에 성능상 훨씬 더 좋은 우위를 차지한다. 
...
btns.forEach((btn, idx) => {
    btn.addEventListener('click', (i) => changeImge(idx)); 
});
...
// 3. 클릭한 요소의 id값을 저장
const thisAttr = btn[idx].getAttribute('id');
const src = `${thisAttr}.jpg`;
...
const h2el = document.childElementCount('h2');
